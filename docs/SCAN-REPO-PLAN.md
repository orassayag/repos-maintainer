...
- It will use the same emoji logic as "Sync Repos" (actually the pre-plan says same emoji as "Sync Repos", which is `♻️`, but I'll use `🔍` as suggested in some parts or stick to the requirement).
  - _Correction_: Pre-plan says "It will have the same emoji in the menu like the 'Sync Repos' option." which is `♻️`. Wait, `Sync Repos` has `♻️` in `src/cli.ts`. I will use `♻️ Scan Repo`.
...
- **Search Logic**:
  ...
  - _Reference_: Logic for suggestions and dropdown will be adapted from existing patterns (or similar projects).
