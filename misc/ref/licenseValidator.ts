...
function detectLicenseType(lower: string): LicenseType {
  ...
  const has = (...phrases: string[]) =>
    phrases.every((p) => includesFuzzy(lower, p));
  ...
  if (
    ...
    return 'MIT';
  if (has('apache license', 'version 2.0', 'licensed under the apache license'))
    return 'Apache-2.0';
  if (
    has('gnu general public license', 'version 3', 'free software foundation')
  )
    return 'GPL-3.0';
  ...
  if (includesFuzzy(lower, 'redistribution and use in source and binary forms'))
    return 'BSD-2-Clause';
...
function extractCopyrightYear(content: string): number | undefined {
  ...
  const rangeMatch = content.match(
    /copyright\s+(?:\(c\)|©|&copy;)?\s*\d{4}-(\d{4})/i
  );
  ...
  for (const phrase of MIT_REQUIRED_PHRASES) {
    if (!includesFuzzy(normalized, phrase)) {
      issues.push({
        ...
        suggestion:
          'Restore the full MIT license text from https://opensource.org/licenses/MIT',
      });
    }
  }
}
  ...
  for (const phrase of APACHE_REQUIRED_PHRASES) {
    if (!includesFuzzy(normalized, phrase)) {
      issues.push({
        ...
        suggestion:
          'Restore the full Apache 2.0 license from https://www.apache.org/licenses/LICENSE-2.0',
      });
    }
  }
}
  ...
  for (const phrase of GPL3_REQUIRED_PHRASES) {
    if (!includesFuzzy(normalized, phrase)) {
      issues.push({
        ...
        suggestion:
          'Restore the full GPL-3.0 license from https://www.gnu.org/licenses/gpl-3.0.txt',
      });
    }
  }
}

function validateBSD(
  normalized: string,
  variant: 'BSD-2-Clause' | 'BSD-3-Clause',
  issues: ValidationIssue[]
): void {
  const phrases =
    variant === 'BSD-3-Clause' ? BSD3_REQUIRED_PHRASES : BSD2_REQUIRED_PHRASES;
  ...
  for (const phrase of ISC_REQUIRED_PHRASES) {
    if (!includesFuzzy(normalized, phrase)) {
      issues.push({
        ...
        suggestion:
          'Restore the full ISC license from https://opensource.org/licenses/ISC',
      });
    }
  }
}
...
function validateStructure(
  ...
  if (!/copyright/i.test(content)) {
    ...
    if (!year) {
      ...
      if (year < 1970) {
        issues.push({
          ...
          message: `Copyright year ${year} looks suspicious.`,
        });
      }
      if (year > currentYear) {
        issues.push({
    ...
    if (!author || author.trim().length === 0) {
      issues.push({
        ...
        suggestion:
          'Add your name or organization after the year: Copyright (c) 2026 Your Name',
      });
    } else {
      const placeholders = [
        'your name',
      ...
      if (placeholders.some((p) => author.toLowerCase().includes(p))) {
        issues.push({
          ...
          suggestion: 'Replace with your actual name or organization.',
        });
      }
    }
  }
  ...
  if (/[^\x00-\x7F]/.test(content)) {
    issues.push({
      ...
      suggestion:
        'Ensure the file is saved as UTF-8 if this is intentional (e.g. © symbol).',
    });
  }
...
export function validateLicense(content: string): LicenseValidationResult {
  ...
  switch (licenseType) {
    case 'MIT':
      validateMIT(normalized, issues);
      break;
    case 'Apache-2.0':
      validateApache(normalized, issues);
      break;
    case 'GPL-3.0':
      validateGPL3(normalized, issues);
      break;
    case 'BSD-2-Clause':
      validateBSD(normalized, 'BSD-2-Clause', issues);
      break;
    case 'BSD-3-Clause':
      validateBSD(normalized, 'BSD-3-Clause', issues);
      break;
    case 'ISC':
      validateISC(normalized, issues);
      break;
  }
...
export function printValidationResult(result: LicenseValidationResult): void {
  ...
  if (result.detectedAuthor)
    console.log(`   Author : ${result.detectedAuthor}`);
