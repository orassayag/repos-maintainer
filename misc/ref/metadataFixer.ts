...
export async function fixMetadata(
  ...
  if (
    !newDescription ||
    newDescription.length < settings.DESCRIPTION_MIN_REPLACE
  ) {
    ...
    changes.push(
      `Description: ${newDescription.length} chars (within acceptable range, left as-is)`
    );
  }
  ...
  if (metadata.topics.length < settings.MIN_TOPICS) {
    ...
    changes.push(
      `Topics: Currently ${metadata.topics.length} (minimum ${settings.MIN_TOPICS})`
    );
  }
